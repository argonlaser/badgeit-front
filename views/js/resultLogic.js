...
//Group By -using Group Key
vm.modifiedInput=_.groupBy(vm.message, 'Group');
...
var vmSelected=new Vue({
	...
	data: {
		...
		changeStyle:function(event)
		{
			...
			alert("style changed to "+this.selectedStyle);
			alert("copy text "+this.copyText);
		},
...
var room = location.pathname.substring(3)
console.log('Room: ' ,  room)
// Listen to '/'
var socket = io.connect(host, {secure: true, forceNew:true});
  socket.on('news', function (data) {
    var badges = data.reqData
    vm.message= badges;
    console.log('Socket : ', socket.id , 'data : ', badges)
